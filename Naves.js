#include "Naves.h"
#include <stdlib.h>
#include <math.h>
#include <stdio.h>
#include <time.h>

#define ACELERACION 250
#define POS_INICIAL_X ANCHO/2
#define POS_INICIAL_Y ALTO/2

struct VectorRep {
	float x;
	float y;
};

typedef struct VectorRep Vector;

struct NaveRep {
	Vector pos;
	Vector pos0;
	Vector v;
	Vector v0;
	Vector a;
	Vector a0;
	Vector t;
	Vector t0;
	float radio;
};

float Modulo ( Vector v ) {
	return sqrt( pow(v.x, 2) + pow(v.y, 2) );
}

float Distancia ( Nave a, Nave b ) {
	return sqrt( pow(a->pos.x - b->pos.x, 2) + pow(a->pos.y - b->pos.y, 2) );
}

int Colision_Nave (Nave a, Nave b) {
	return (Distancia (a, b) < (a->radio + b->radio));
}

Vector Aceleracion (Vector a, float moduloa) {
	float mod = Modulo(a);
	a.x = moduloa*a.x/mod;
	a.y = moduloa*a.y/mod;
	return a;
}


void Cargar_Imagenes(char *nombre, Imagen array[]) {
	for (int i = 0 ; i <= 359 ; i++) {
		char ruta[80];
		sprintf(ruta, "Img/%s/%s%03d.bmp", nombre, nombre, i);
		array[i] = Pantalla_ImagenLee(ruta, 1);
	}
}

void Libera_Imagenes(Imagen array[]) {
	for (int i = 0 ; i <= 359 ; i++) {
		Pantalla_ImagenLibera(array[i]);
	}

}

int Angulo_Nave (Nave n) {
	float mod = Modulo(n->v);
	int angulo = asin(n->v.y/mod)*180/M_PI;
	if (angulo <= 0 && n->v.x >= 0) angulo = -angulo;
	else if (angulo <= 0 && n->v.x < 0) angulo = 180 + angulo;
	else if (angulo > 0 && n->v.x < 0) angulo = 180 + angulo;
	else angulo = 360 - angulo;
	return angulo;
}



void Valores_Iniciales(Nave nave) {
	if (nave->a.x != nave->a0.x) {
		nave->pos0.x = nave->pos.x;
		nave->v0.x = nave->v.x;
		nave->t0.x = (float)clock() - 40;
		nave->a0.x = nave->a.x;
	}
	if (nave->a.y != nave->a0.y) {
		nave->pos0.y = nave->pos.y;
		nave->v0.y = nave->v.y;
		nave->t0.y = (float)clock() - 40;
		nave->a0.y = nave->a.y;
	}
}

void Actualiza_Posicion (Nave nave) {
	nave->v.x = nave->v0.x + nave->a.x*nave->t.x;
	nave->pos.x = nave->pos0.x + nave->v0.x*nave->t.x + (nave->a.x*nave->t.x*nave->t.x)/2;

	nave->v.y = nave->v0.y + nave->a.y*nave->t.y;
	nave->pos.y = nave->pos0.y + nave->v0.y*nave->t.y + (nave->a.y*nave->t.y*nave->t.y)/2;
}



Nave Crea_Bala (Nave n) {
	Nave nueva = malloc ( sizeof(struct NaveRep));
	Vector v;
	v.x = n->v.x/Modulo (n->v);
	v.y = n->v.y/Modulo (n->v);
	nueva->v.x = v.x*50;
	nueva->v.y = v.y*50;
	nueva->pos.x = n->pos.x;
	nueva->pos.y = n->pos.y;
	nueva->radio = 10;
	return nueva;
}

Nave Crea_Nave(int tipo) {
	Nave nuevo = malloc ( sizeof(struct NaveRep));
	if (tipo) {
	int lado = rand()%4;
		switch(lado) {
			case 0: //arriba
				nuevo->pos.x = rand()%ANCHO;
				nuevo->pos.y = -30;
				break;
			case 1://abajo
				nuevo->pos.x = rand()%ANCHO;
				nuevo->pos.y = ALTO + 30;
				break;
			case 2://izquierda
				nuevo->pos.x = -30;
				nuevo->pos.y = rand()%ALTO;
				break;
			case 3://derecha
				nuevo->pos.x = ANCHO + 30;
				nuevo->pos.y = rand()%ALTO;
				break;
		}
	nuevo->pos0 = nuevo->pos;
	} else {
		nuevo->pos.x = POS_INICIAL_X;
		nuevo->pos.y = POS_INICIAL_Y;
		nuevo->pos0 = nuevo->pos;
	}
	nuevo->radio = 30;
	nuevo->v.x = 0;
	nuevo->v.y = 0;
	nuevo->v0 = nuevo->v;
	nuevo->a = nuevo->v;
	nuevo->a0 = nuevo->v;
	nuevo->t = nuevo->v;
	nuevo->t0 = nuevo->v;
	return nuevo;
}

void Libera_Nave(Nave n) {
	free(n);
}

float Get_NavePosicionX (Nave n) {
	return n->pos.x;
}

float Get_NavePosicionY (Nave n) {
	return n->pos.y;
}


void Actualiza_Nave(Nave n, float x, float y) {

	n->a.x = x - n->pos.x;
	n->a.y = y - n->pos.y;

	if (n->a.y || n->a.x) {
		n->a = Aceleracion(n->a, ACELERACION);
	}

	Valores_Iniciales(n);

	n->t.x = ((float)clock() - n->t0.x)/1000;
	n->t.y = ((float)clock() - n->t0.y)/1000;

	Actualiza_Posicion (n);
}

void Limite_Pantalla (Nave n) {
	if (n->pos.x < 0+30) {
		n->pos0.x = 30;
		n->a.x = 50;
		n->v0.x = 50;
	} else if (n->pos.x > Pantalla_Anchura()-30) {
		n->pos0.x = Pantalla_Anchura()-30;
		n->a.x = -50;
		n->v0.x = -50;
	}

	if (n->pos.y < 0+30) {
		n->pos0.y = 30;
		n->a.y = 50;
		n->v0.y = 50;
	} else if (n->pos.y > Pantalla_Altura()-30) {
		n->pos0.y = Pantalla_Altura()-30;
		n->a.y = -50;
		n->v0.y = -50;
	}
}



void Actualiza_NavePrincipal(Nave n, float x, float y) {
	n->a.x = x - n->pos.x;
	n->a.y = y - n->pos.y;

	if (n->a.y || n->a.x) {
		if (Pantalla_TeclaPulsada(SDL_SCANCODE_SPACE)) {
			n->a = Aceleracion(n->a, 4000);
		} else {
			n->a = Aceleracion(n->a, 750);
		}
	}

	Valores_Iniciales(n);

	n->t.x = ((float)clock() - n->t0.x)/1000;
	n->t.y = ((float)clock() - n->t0.y)/1000;

	Limite_Pantalla(n);

	Actualiza_Posicion (n);


}

void Actualiza_Balas (Nave n) {
	n->pos.x = n->pos.x + n->v.x;
	n->pos.y = n->pos.y + n->v.y;
}





void Dibuja_Nave (Nave n, Imagen img[]) {
	int angulo = Angulo_Nave(n);
	double ancho = Pantalla_ImagenAnchura(img[angulo]);
	double alto = Pantalla_ImagenAltura(img[angulo]);
	Pantalla_DibujaImagen(img[angulo], n->pos.x-ancho/2, n->pos.y-alto/2, ancho, alto);
}








